...
class EmailSkill(Skill):
    ...
    def get_tools(self) -> List[BaseTool]:
        ...
        def setup_email(email_address: str, password: str, imap_server: str, smtp_server: str, account_name: str = "default") -> str:
            ...
            try:
                ...
                if account_name == "default":
                    ...
                    if len(files) == 1:
                        target_file_name = files[0].replace(".json", "")
                    elif len(files) > 1:
                                ...
                                try:
                                    with open(os.path.join(self.config_dir, fname), "r") as f:
                                        ...
                                        if data.get("EMAIL_ADDRESS", "").lower() == email_address.lower():
                                            target_file_name = fname.replace(".json", "")
                                            found_match = True
                                            break
                                except:
                                    pass
        ...
        @tool
        def search_emails(query: str, k: int = 5) -> str:
            """
            Search archived emails by semantic similarity.
            Args:
                query: Search query (e.g., "invoice from last week").
            ...
            for i, doc in enumerate(results):
                ...
                output.append(f"{i+1}. From: {sender} | Subject: {subject} | Date: {date}\n   Snippet: {snippet}...")
            
            return "\n".join(output)

        return [setup_email, check_inbox, send_email, download_emails, search_emails]
